...
with open(source_file, 'rb') as bat:  # No encoding
    ...
    print(f'The object type of `file_header` is: {type(file_header)}')
    ...
    if bmp_id == b'BM':  # we have a Windows bitmap file
        ...
        if file_size != os_size:
            ...
            if dib_header_size == 40:
                ...
                image = bytearray(bat.read(pixel_array_size))   # Because we're modifying the bytes at each position
                # in the array, we have to use a bytearray rather than a bytes object. bytes are immutable.
                # A bytearray is mutable version of bytes
                print(f'The object type of `image` is: {type(image)}')
